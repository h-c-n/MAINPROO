...
db = MySQLdb.connect('localhost','root','','credit')
c = db.cursor()
...
def SignIn(request):  
    ...
    if request.POST:
        ...
        if ds[2] == 'Admin':
            ...
            c.execute("select * from club_reg where email='"+email+"' and password='"+password+"'")
            ds = c.fetchone()
            request.session['cid'] = ds[0]
            return HttpResponseRedirect('/ClubHome/')
        elif ds[2] == 'Shop':
            c.execute("select * from shop_reg where email='"+email+"' and password='"+password+"'")
            ds = c.fetchone()
...
def AdminRemoveWorkshop(request):
    if request.GET:
        ...
        db.commit()
        return HttpResponseRedirect("/Admin_Add_Workshop/")
    return render(request,'AdminRemoveWorkshop.html')

def addcard(request):
    if request.POST:
        cardno=request.POST.get("t1")
        pin=request.POST.get("t2")
        cvv=request.POST.get("t3")
        dox=request.POST.get("t4")
        dob=request.POST.get("t5")
        uname=request.session["username"]
        c.execute("insert into bank (`cardno`,`pin`,`cvv`,`dateofexp`,`dob`,`status`,`username`) values('"+str(cardno)+"','"+str(pin)+"','"+str(cvv)+"','"+str(dox)+"','"+str(dob)+"','Unblocked','"+str(uname)+"')")
        db.commit()
        
    return render(request,'addcard.html')
def viewcardstatus(request):
    uname=request.session["username"]
    data = ""
    c.execute("select * from bank where username='"+str(uname)+"'")
    data=c.fetchall() 
    return render (request,"viewcardstatus.html",{"data":data})
def Adminviewcardstatus(request):
    
    data = ""
    c.execute("select * from bank ")
    data=c.fetchall() 
    return render (request,"Adminviewcardstatus.html",{"data":data})
def Unblock(request):
    cardno=request.GET.get("cardno")
    data = ""
    c.execute("update bank set Status='Unblocked' where cardno='"+cardno+"'" )
    db.commit()
    return HttpResponseRedirect("/Adminviewcardstatus/")
    return render (request,"Adminviewcardstatus.html",{"data":data})
...
def Admin_Add_Brand(request):
    ...
    if request.POST:
        na = request.POST.get("cat_name")
        qry="insert into brands(brand_title) values('"+ na +"')"
        print(qry)
        c.execute(qry)
        db.commit()
...
def Admin_View_Feedback(request):
    data = ""
    c.execute("SELECT feedback.*, cust_reg.* FROM feedback INNER JOIN cust_reg on cust_reg.cid = feedback.userid")
    data = c.fetchall()
    if request.GET:
        a = request.GET.get('id')
...
def Customer_View_Profile(request):
    ...
    if request.session['uid']:   
        ...
        if request.POST:
            ...
            c.execute("update cust_reg set mobile='"+Phn+"' and address='"+Address+"' where cid='"+str(uid)+"'")
            db.commit()
...
def CustomerViewCart(Request):
    ...
    if Request.GET:
        ...
        c.execute("delete from cart where cartid = '"+str(ci)+"'")
        db.commit()
        return HttpResponseRedirect("/CustomerViewCart")
    if(Request.POST):
...
def payment1(request):
    ...
    try:
        ...
        if request.POST:
            ...
            ss="select * from bank where cardno='"+str(cardno)+"' "
            print(ss)
            c.execute(ss)
            data=c.fetchall()
            data1=data[0][1]
            ...
            if data:
                if((data[0][1]==pinno) and (data[0][5]=="Unblocked")):  
                    msg="card validation success"
                    return HttpResponseRedirect("/payment2")
                elif(data[0][1]!=pinno):
                    msg="invalid pin"
                    count=int(request.session["count"])+1
                    request.session["count"]=count
                    print("hiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii")
                    print(count)
                    if(count>=3):

                            msg="card blocked for 48 hrs"
                            ss="update bank set status='"+msg+"' where cardno='"+str(cardno)+"'"
                            print(ss)
                            c.execute(ss)
                            db.commit()
                    elif(data[0][1]==pinno[::-1]):
                        msg="Suspesious entry card blocked for 24 hrs"
                        ss="update bank setstatus='"+msg+"' where cardno='"+str(cardno)+"'"
                        c.execute(ss)
                        db.commit()
                    elif(set(data1).issubset(pinno)):
                        count=int(request.session["count"])+1
                        msg="card blocked for 3 days"
                        ss="update bank setstatus='"+msg+"' where cardno='"+str(cardno)+"'"
                        c.execute(ss)
                        db.commit()
                else:
                    msg="Card Blocked Pls View Card Status"
